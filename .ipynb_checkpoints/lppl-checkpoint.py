...
class chromosomes:
    ...
    # evaluate the fitness of the paramete    try:
            # evaluate MSE with the para        
            the LPPL function
            fitn(ss = error_func(self.parameters)
     )       if self.verbose:
            ...
            umber_of_functions_evaluated = nfevl
            self.message = opt.tnc.RCSTRINGS[rc]
  (         self.fitness = error_func(self.para)meters)
            if self.verbose:
...
class population:
    ...
    # breed chromosomes
    de("Breeding chromosomes: "+str(i+1)+" / "+str(self.max_size))somes reaches the max size
    ...
    def eliminate(self):
     ...
     ("Survived: "+str(size)+" -> "+str(len(self.pool))+" / "+str(size))x_size * (1.0-self.eliminate_ratio))
    ...
    # evolve
    def evolve(se("--- ["+str(i+1)+"/"+str(num_generations)+"] current best performers ---")n xrange(num_generations):
          self.breed()
          self.crossover()
          self.mate()
    (j)    self.mutate(("\n")        self.eliminate()
...
def draw_single_step(p, all_data, learning_data):
    ...
    learning_end_p("--- RESULT ---")eseries[-1]
    # plot the pre-learning and learning part of the actual data(str(x))lt.scatter(timeseries_all[:learning_end_pos],
...
def multi_steps(ticker, start_date, end_date, max_term, min_term, prediction_term, generations, verbose):
    ...
    for learning_end_date in datetimes_all[max_term:]:
      ...
      LEARNING_DATA = get_learning_data(all_data, learning_end_date, max_term)
      (TIMESERIES, ACTUAL_VALUES, DATETIMES)("--- RESULT ---", DATETIMES[-1])eters = generate_ga_parameters(TIMESERIES, min_term=min_term)
      # (str(x)) the ga process
